#!/usr/bin/env python3
import sys, os, re, json, time, threading, random, tempfile, base64, uuid, shutil, webbrowser, requests, yt_dlp, vlc
from urllib.parse import urljoin, urlencode, unquote
from bs4 import BeautifulSoup
from cryptography.fernet import Fernet
from cryptography.hazmat.primitives import hashes
from cryptography.hazmat.primitives.kdf.pbkdf2 import PBKDF2HMAC
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QSplitter, QLineEdit, QPushButton, QLabel, QSlider, QScrollArea, QListWidget, QListWidgetItem, QMessageBox, QFileDialog, QInputDialog, QSystemTrayIcon, QMenu, QAction, QFrame, QStyleFactory)
...
cookies_salt_file = "soundcloud_cookies.salt"
...
def get_machine_key():
    mac = uuid.getnode()
    if mac == 0xffffffffffff or mac == 0:
...
def load_plain_cookies():
    if os.path.exists(plain_cookies_file):
        return plain_cookies_file
    return None
...
def find_media_url(page_url):
    ...
    direct_mp3 = soup.find('a', href=re.compile(r'\.mp3$', re.I))
    if direct_mp3:
        return urljoin(page_url, direct_mp3['href'])
...
class TrackWidget(QFrame):
    ...
    def init_ui(self):
        ...
        if track_type == 'remix' and artist.lower().startswith('arranged by'):
            if artist[11:12] != ' ':
                artist = artist[:11] + ' ' + artist[11:]
        if track_type == 'album':
            album = self.track.get('album_name', '')
            if album.startswith('Album: '):
                album = album[7:]
            prefix = f"[A={album}] " if album else "[A] "
        elif track_type == 'remix':
            prefix = "[R] "
        else:
            prefix = "[O] "
        ...
        self.label.mousePressEvent = lambda e: webbrowser.open(self.track['url'])
...
class GameWidget(QFrame):
    ...
    def toggle_collapse(self):
        ...
        if self.collapsed:
            self.track_container.hide()
            self.arrow_button.setText("▶")
        else:
            self.track_container.show()
            self.arrow_button.setText("▼")
...
class OCRemixWindow(QMainWindow):
    def __init__(self):
        ...
        self.signals.skip_to_next.connect(self.skip_to_next)
        self.signals.skip_global_shuffle_next.connect(self.skip_global_shuffle_next)
    ...
    def setup_ui(self):
        ...
        ctrl_layout.addWidget(self.shuffle_button)
        ...
        ctrl_layout.addWidget(self.global_shuffle_button)
        ...
        ctrl_layout.addWidget(self.repeat_button)
        ...
        file_menu.addAction("Load Queue", self.load_queue)
    ...
    def scrape_all_games(self):
        ...
        for filt in filters:
            offset = 0
            while True:
                params = {"sort": "", "filter": filt, "offset": offset}
    ...
    def filter_games(self):
        ...
        if not self.game_catalog:
            ...
            self.filtered_game_ids = [
                gid for gid, data in self.game_catalog.items()
    ...
    def adjust_game_list_width(self):
        ...
        width = min(max_len * 9 + 30, 350)
        self.game_list.setMinimumWidth(width)
    ...
    def load_tracks_incremental(self, game_id):
        ...
        try:
            for t in self.scrape_stream(remix, '/remix/', headers):
                process(t)
        except Exception as e:
        ...
        try:
            ...
            for a in soup.find_all('a', href=lambda h: h and '/album/' in h):
                ...
                album_url = urljoin(main, a['href'])
                items.append((album_url, album_name))
        ...
        self.signals.status_message.emit(f"Added {self.game_catalog[game_id]['name']} ({count} tracks)")
    ...
    def scrape_stream(self, page_url, link_pattern, headers):
        ...
        for a in soup.find_all('a', href=lambda h: h and link_pattern in h):
            ...
            preview = a.get('data-preview', '').strip()
    ...
    def scrape_album(self, album_url, headers, album_name=None):
        ...
        if table:
            ...
            for row in table.find_all('tr'):
                ...
                preview = ''
                play = row.find('a', class_='play-button') or row.find('a', {'data-preview': True})
                if play and play.get('href'):
                    preview = urljoin(album_url, play['href'])
                tracks.append({
                    'title': title,
                    'artist': artist,
                    'url': track_url,
    ...
    def update_loading_display(self):
        ...
        lines = [f"Loading {info['name']}: {info['count']} tracks" for info in self.loading_tasks.values()]
    ...
    def clear_queue(self):
        if not self.added_games:
            return
        reply = QMessageBox.question(self, "Clear Queue", "Remove all games and tracks from the queue?", QMessageBox.Yes | QMessageBox.No)
    ...
    def save_queue(self):
        ...
        for gid, info in self.added_games.items():
            tracks = self.tracks_by_game.get(gid, [])
            serial = []
            for t in tracks:
                serial.append({
                    'title': t['title'],
        ...
        for game in data:
            ...
            for tdata in game['tracks']:
                track = {
                    ...
                    'artist': tdata['artist'],
                    'url': tdata['url'],
                    'preview_url': tdata.get('preview_url', ''),
                    'type': tdata.get('type', 'original'),
                    'album_name': tdata.get('album_name')
                }
    ...
    def _play_track(self, track):
        ...
        def worker():
            ...
            try:
                ...
                if preview and not is_soundcloud_profile(preview):
                    media_url = preview
                else:
                    media_url = find_media_url(track['url'])
    ...
    def start_playback(self, track, stream_url, duration):
        ...
        game = self.added_games.get(game_id, {}).get('name', '')
        if game:
            text = f"Now playing: {game} - {track['title']} — {track['artist']}"
        else:
            text = f"Now playing: {track['title']} — {track['artist']}"
    ...
    def skip_global_shuffle_next(self):
    ...
    def start_shuffle(self):
        ...
        if self.last_played_track and len(tracks) > 1:
            if tracks[0] == self.last_played_track:
        ...
        if len(self.shuffle_queue) > 1 and next_track == self.last_played_track:
            for offset in range(1, len(self.shuffle_queue)):
                ...
                if self.shuffle_queue[cand] != self.last_played_track:
                    self.shuffle_queue[self.shuffle_index], self.shuffle_queue[cand] = \
                        self.shuffle_queue[cand], self.shuffle_queue[self.shuffle_index]
                    next_track = self.shuffle_queue[self.shuffle_index]
                    break
    ...
    def on_previous(self):
        ...
        prev = self.play_history[-1]
        self._play_track(prev)
        ...
        try:
            ...
            for a in soup.find_all('a', href=lambda h: h and '/album/' in h):
                ...
                if parent and 'Promotion' in parent.get_text():
                    continue
                name = a.get_text(strip=True)
                url = urljoin(main, a['href'])
                items.append((url, name))
            for idx, (url, name) in enumerate(items):
                if idx > 0:
                    time.sleep(2)
                try:
                    album_tracks = self.scrape_album(url, headers, name)
                    entries.extend(album_tracks)
                except:
                    pass
        except:
            pass
    ...
    def seek(self, value):
        if not self.player.current_url or self.player.length <= 0:
            return
    ...
    def update_progress(self):
        if self.player.is_playing():
            ...
            if length > 0 and pos >= 0:
                ...
                if self.current_track:
                    self.status_label.setText(f"Now playing: {self.current_track['title']} — {self.current_track['artist']}  [{cur} / {total}]")
    ...
    def manage_cookies(self):
        ...
        reply = QMessageBox.question(self, "How to get SoundCloud cookies", instructions, QMessageBox.Ok | QMessageBox.Cancel)
        if reply != QMessageBox.Ok:
            return
        filepath, _ = QFileDialog.getOpenFileName(self, "Select SoundCloud cookies.txt", "", "Text files (*.txt)")
        if not filepath:
            return
        choice = QMessageBox.question(self, "Storage Method", "Yes = Auto‑Encrypt (machine‑bound, no password)\nNo = Plain Text (no security)", QMessageBox.Yes | QMessageBox.No)
        if choice == QMessageBox.Yes:
            if encrypt_cookies_auto(filepath):
                delete_orig = QMessageBox.question(self, "Delete Original?", "Delete the plaintext file?", QMessageBox.Yes | QMessageBox.No)
                if delete_orig == QMessageBox.Yes:
                    try:
                        os.remove(filepath)
                    except:
                        pass
    ...
    def on_status_click(self, event):
        if self.current_track:
            webbrowser.open(self.current_track['url'])
